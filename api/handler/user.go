...
func (h *Handler) CreateUser(c *gin.Context) {
	...
	if err != nil {
		h.handlerResponse(c, "storage.getbyid.user", http.StatusInternalServerError, err.Error())
		return
	}
...
func (h *Handler) GetByIdUser(c *gin.Context) {
	...
	resp, err := h.storages.User().GetById(&models.UserPrimaryKey{Id: id})
	if err != nil {
		h.handlerResponse(c, "storage.user.get by id", http.StatusInternalServerError, err.Error())
		return
	}

	h.handlerResponse(c, "get by id user", http.StatusOK, resp)
}
...
func (h *Handler) UpdateUser(c *gin.Context) {
	...
	resp, err := h.storages.User().GetById(&models.UserPrimaryKey{Id: updateUser.Id})
	if err != nil {
		h.handlerResponse(c, "storage.getbyid user", http.StatusInternalServerError, err.Error())
		return
	}

	h.handlerResponse(c, "update user response", http.StatusAccepted, resp)
}
...
func (h *Handler) DeleteUser(c *gin.Context) {
	...
	if !helper.IsValidUUID(id) {
		h.handlerResponse(c, "delete user", http.StatusBadRequest, "invalid user id")
		return
	}

	err := h.storages.User().Delete(&models.UserPrimaryKey{Id: id})
	if err != nil {
